...
def print_rust(state_size, rc, mds, f):
  assert(len(rc) % state_size == 0)
  assert(mds.nrows() == state_size)
  assert(mds.ncols() == state_size)
  steps = len(rc) // state_size
  f.write("pub static ref RC{}: Vec<Vec<Scalar>> = vec![".format(state_size))
  ...
  f.write("pub static ref MDS{}: Vec<Vec<Scalar>> = vec![".format(state_size))
...
# Parameters
  ...
  with open('rc_poseidon_{}'.format(t), 'w') as f:
    ...
    print_rust(t, round_constants, MDS, f)
